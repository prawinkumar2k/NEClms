           ...
           <div className="lg:col-span-2 space-y-4">
              <Card className="rounded-[40px] overflow-hidden glass border-white/5 bg-black shadow-3xl">
                 <div className="aspect-video relative group">
                    {(activeFrame || sortedFrames[0]) ? (
                       <img src={activeFrame?.screenshot || sortedFrames[0]?.screenshot} className="w-full h-full object-contain" alt="Current Frame" />
                    ) : (
              ...
              {/* Controls & Details */}
              <div className="flex items-center justify-between p-6 glass rounded-[32px] border-white/5">
                 ...
                 <div className="flex gap-2">
                    <Button variant="ghost" className="rounded-xl h-10 w-10 p-0 hover:bg-white/10"><Play className="w-4 h-4" /></Button>
                    <Button variant="ghost" className="rounded-xl h-10 w-10 p-0 hover:bg-white/10 text-rose-500"><AlertTriangle className="w-4 h-4" /></Button>
                 </div>
              </div>
           </div>
                 ...
                 <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                    {sortedFrames.map((frame, idx) => (
                       <button 
                          ...
                          className={`w-full flex items-center gap-3 p-3 rounded-2xl transition-all border ${activeFrame?._id === frame._id ? 'bg-primary/10 border-primary/20 shadow-lg' : 'bg-white/5 border-transparent hover:bg-white/10'}`}
                       >
                          <div className="w-16 aspect-video rounded-lg overflow-hidden shrink-0 border border-white/10 bg-black">
                             <img src={frame.screenshot} className="w-full h-full object-cover" alt="ts" />
                          </div>
