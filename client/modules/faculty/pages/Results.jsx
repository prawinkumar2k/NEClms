...
export default function FacultyResults() {
  ...
  const { data: results, isLoading } = useQuery({
    ...
    queryFn: () => apiClient.get("/api/submissions"), // Or a specific results endpoint
  });
            ...
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  filtered.map((r) => (
                    <TableRow key={r._id} className="border-white/5 hover:bg-white/5 transition-colors group">
                      <TableCell className="py-6 pl-10">
                        <p className="font-bold text-sm tracking-tight">{r.student?.name}</p>
                      ...
                      <TableCell className="text-center">
                         <Badge className={`rounded-full px-4 py-1.5 text-[9px] font-black uppercase tracking-widest border-none ${r.status === 'submitted' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-muted text-muted-foreground'}`}>
                           {r.status}
                         </Badge>
                      </TableCell>
                      <TableCell className="pr-10 text-right">
                        <div className="flex justify-end gap-2">
                          <Button 
                            ...
                            onClick={() => navigate(`/faculty/evidence/${r.exam?._id || r.exam}/${r.student?._id || r.student}`)}
                          >
                             <Play className="w-3.5 h-3.5 mr-2" /> Recording
                          </Button>
                          <Button variant="ghost" size="sm" className="w-10 h-10 rounded-xl hover:bg-primary/10 hover:text-primary transition-all">
